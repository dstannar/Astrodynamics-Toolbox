...
def scanLambertCandidates(Orbit1s, Orbit2s, dt=100, bestGuess=None):
    ...
    Input:
        Orbit1s - List of orbit objects to transfer from
        Orbit2s - List of orbit objects to transfer to
    ...
    Output (bestResult array):
        ...
        [13] v_pretrans1, [14] v_pretrans2 - both km/s, true lambert velocities at each point
    ...
    dvMagOld = np.inf
    bestResult = None
    # if preseeded choose correct NORAD IDs
    ...
    for orbit1 in Orbit1s:
        for orbit2 in Orbit2s:
            # if they are the same debris object move on
            if orbit1.satnum == orbit2.satnum:
                continue
            # ensure we only look at already determined best objects (lowest dv transfers) from preseed
            ...
            while t1 <= t1_end:
                ...
                # update t2 start so we don't try to transfer backwards in time
                if t2 < t1:
                    t2 = t1
                ...
                while t2 <= t2_end:
                    tof = t2 - t1
                    # check in case
                    if np.sign(tof) == np.sign(-1):
                        print('uh oh negative time')
                    ...
                    # velocities returned by Lamberts
                    v_pretrans1 = v1New
                    v_pretrans2 = v2New
                    ...
                    if np.isfinite(dvMag) and dvMag < dvMagOld:
                        ...
                        if alt_per >= 100.0 ** np.isfinite(alt_per):
                            dvMagOld = dvMag
                            bestResult = [
                                ...
                                v_pretrans1, v_pretrans2 # lambert velocities, km/ss
                            ]
...
def plane_velo_change_and_phase(orbit1, orbit2):
    ...
    # Burn #1 at GEO node
    r_now, v_now = Propagate(t1_rel, Orbit=satOrbit).lagrange_coeff()
    ...
    r_end_tr, v_end_tr = Propagate(t_half, Orbit=satOrbit).lagrange_coeff()
    ...
    r2_phase, v2_phase = Propagate(prop_time=missionTime, Orbit=orbit2).lagrange_coeff()
    ...
    sma_phase = (np.sqrt(mu)*t_toPerigee / (2*np.pi))**(2/3)
    ...
    dv3_mag = h_phase / orbit2.r_per
    dv4_mag = dv3_mag
    # get velocity of orbit2 at perigee (don't set state), get unit vector of velocity, multiply by dv mags above for dv vectors
    r2_per, v2_per = Propagate(prop_time=t_toPerigee, Orbit=orbit2).lagrange_coeff()
    # v2 unit vector
    vp_hat = v2_per / np.linalg.norm(v2_per)
    # get dv vectors
    vEnter_phase = vp_hat * -dv3_mag # slowing down
    vExit_phase = vp_hat * dv4_mag # get back into normal orbit
    ...
    rp_sat, vp_sat = Propagate(prop_time=t_toPerigeeSAT, Orbit=satOrbit).lagrange_coeff()
    satOrbit.set_state(rp_sat, vp_sat)
    # update mission time
    missionTime += t_toPerigeeSAT
    # do burn 1 into phasing orbit and set state
    satOrbit.set_state(satOrbit.r, satOrbit.v + vEnter_phase)
    # propagate for one phase period, set state, update mission timer
    r_afterPhase, v_afterPhase = Propagate(prop_time=satOrbit.period, Orbit=satOrbit).lagrange_coeff()
    missionTime += satOrbit.period
    satOrbit.set_state(r_afterPhase, v_afterPhase)
    # check that we got to perigee at same time and position as orbit2 debris
    # propagate orbit2 with time = t_toPerigee noting that t_toPerigee = orbitSat.period if orbitSat is on phase orbit
    r2_afterPhase, v2_afterPhase = Propagate(prop_time=t_toPerigee, Orbit=orbit2).lagrange_coeff()
    orbit2.set_state(r2_afterPhase, v2_afterPhase)
    burn3_check = np.isclose(satOrbit.r, orbit2.r).all()
    print(np.linalg.norm(satOrbit.r - orbit2.r))
    print('[BURN 3 ACCURACY CHECK (T/F)]: ', burn3_check)

    # do burn 4
    satOrbit.set_state(satOrbit.r, satOrbit.v + vExit_phase)
    # check accuracy
    burn4_check = np.isclose(satOrbit.r, orbit2.r).all() and np.isclose(satOrbit.v, orbit2.v).all()
