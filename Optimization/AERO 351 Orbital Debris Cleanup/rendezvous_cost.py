...
def get_rendezvous_cost(geo_id: int, meo_id: int, leo1_id: int, leo2_id: int) -> float:
    """
    Returns delta v cost for rendezvous sequence. Transfer types are hardcoded.
    GEO -> MEO: apse line rotation, combined hohmann/plane change, phasing maneuver
    MEO -> LEO & LEO -> LEO: Lambert's problem maneuver
    """
...
def get_rendezvous(G2M=True, M2L=True, L2L=True, full_soln=True):
    ...
    LEODebrisTLE = [
        40933, 40935, 40932, 40934, 35578, 37840, 25791, 26561
    ]
    LEOODebrisTLE2 = [40933]
